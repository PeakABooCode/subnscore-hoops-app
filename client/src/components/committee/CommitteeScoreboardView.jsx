...
export default function CommitteeScoreboardView() {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950 text-white flex flex-col font-sans overflow-hidden p-4 md:p-8">
      {/* Main Scoreboard Area */}
      <div className="flex-1 flex flex-col lg:flex-row items-center justify-between gap-4 lg:gap-0">
        {/* Team A */}
        <div className="flex flex-col items-center space-y-2 lg:space-y-4 w-full lg:w-1/3">
          ...
          <div className="text-[8rem] md:text-[12rem] lg:text-[16rem] font-mono font-black leading-none tabular-nums text-blue-500 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]">
            {data.scores.A}
          </div>
        ...
        <div className="flex flex-col items-center justify-center space-y-4 lg:space-y-8 w-full lg:w-1/3">
          ...
          <div className="flex flex-col items-center gap-2 lg:gap-4">
            {/* Game Clock */}
            <div className="text-[6rem] md:text-[8rem] lg:text-[11rem] font-mono font-black tabular-nums bg-black px-6 py-1 lg:px-10 lg:py-2 rounded-2xl lg:rounded-3xl border-2 lg:border-4 border-zinc-900 text-amber-500 drop-shadow-[0_0_15px_rgba(245,158,11,0.4)] leading-none">
              {formatTime(data.clock)}
            </div>

            {/* Shot Clock - LARGE and FLASHING */}
            <div className="flex flex-col items-center -mt-2 lg:mt-0">
               <div className={`text-[8rem] md:text-[10rem] lg:text-[14rem] font-mono font-black tabular-nums leading-none transition-colors duration-300 ${data.shotClock <= 5 ? 'text-red-600 animate-pulse' : data.shotClock <= 10 ? 'text-red-500' : 'text-amber-500'}`}>
                 {data.shotClock}
               </div>
        ...
        <div className="flex flex-col items-center space-y-2 lg:space-y-4 w-full lg:w-1/3">
          ...
          <div className="text-[8rem] md:text-[12rem] lg:text-[16rem] font-mono font-black leading-none tabular-nums text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.5)]">
            {data.scores.B}
          </div>
          <div className="space-y-4 w-full flex flex-col items-center">
            {/* Team B Fouls */}
